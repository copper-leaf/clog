...
inline fun Clog.addLogger(logger: ClogLogger, filter: ClogFilter? = null) {
    updateProfile {
        val delegatingLogger = it.logger as? DelegatingLogger
            ?: DelegatingLogger(listOf(DelegatingLogger.FilteredLogger(null, it.logger)))
        it.copy(logger = delegatingLogger + DelegatingLogger.FilteredLogger(filter, logger))
    }
}
