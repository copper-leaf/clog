package com.caseyjbrooks.cloglint;
...
public final class ImproperClogUsageDetector extends Detector implements Detector.JavaScanner,
        Detector.ClassScanner {
    private final static String GET_STRING_METHOD = "getString";
    private final static String CLOG_TREE_LOG_METHOD_REGEXP = "(v|d|i|w|e|wtf)";
    ...
    public void visitMethod(@NonNull JavaContext context, AstVisitor visitor,
        ...
        if ("format".equals(methodName)) {
            ...
            // Look outside to see if we inside of a Clog call
            ...
            if (!Pattern.matches("^Clog\\." + CLOG_TREE_LOG_METHOD_REGEXP + ".*", statement.toString())) {
                return;
            }
            context.report(ISSUE_FORMAT, node, context.getLocation(node),
                    "Using 'String#format' inside of 'Clog'");
        } else if ("tag".equals(methodName)) {
            VariableReference ref = (VariableReference) node.astOperand();
            if (!"Clog".equals(ref.astIdentifier().astValue())) {
                return;
            }
        ...
        } else if (isAstOperandClogTagLogPattern(node)) {
    ...
    /**
     * Rough guess as to whether node represents a `Clog.tag(TAG).v(...)` style log experssion.
     * <p>
     ...
     */
    private static boolean isAstOperandClogTagLogPattern(MethodInvocation node) {
        ...
        if (!"Clog".equals(m.rawOperand().toString())
                || !"tag".equals(m.astName().toString())
                || !Pattern.matches(CLOG_TREE_LOG_METHOD_REGEXP, node.astName().getDescription())) {
            // Is not of the form "Clog.tag(...).w(...)" (where "w()" can be any valid log method)
            return false;
        }
        ...
        if (argument instanceof BinaryExpression) {
            ...
            if (argumentType == String.class) {
                context.report(ISSUE_BINARY, node, context.getLocation(argument),
                        "Replace String concatenation with Clog's string formatting");
                return true;
            }
    ...
    public static final Issue ISSUE_LOG =
            Issue.create("LogNotClog", "Logging call to Log instead of Clog",
                    "Since Clog is included in the project, it is likely that calls to Log should instead"
                            + " be going to Clog.", Category.MESSAGES, 5, Severity.WARNING,
                    new Implementation(ImproperClogUsageDetector.class, Scope.CLASS_FILE_SCOPE));
    public static final Issue ISSUE_FORMAT =
            Issue.create("StringFormatInClog", "Logging call with Clog contains String#format()",
                    "Since Clog handles String.format automatically, you may not use String#format().",
                    Category.MESSAGES, 5, Severity.WARNING,
                    new Implementation(ImproperClogUsageDetector.class, Scope.JAVA_FILE_SCOPE));
    public static final Issue ISSUE_THROWABLE =
            Issue.create("ThrowableNotAtBeginning", "Exception in Clog not at the beginning",
                    "In Clog you have to pass a Throwable at the beginning of the call.", Category.MESSAGES,
                    5, Severity.WARNING,
                    new Implementation(ImproperClogUsageDetector.class, Scope.JAVA_FILE_SCOPE));
    public static final Issue ISSUE_BINARY =
            Issue.create("BinaryOperationInClog", "Use String#format()",
                    "Since Clog handles String#format() automatically, use this instead of String"
                            + " concatenation.", Category.MESSAGES, 5, Severity.WARNING,
                    new Implementation(ImproperClogUsageDetector.class, Scope.JAVA_FILE_SCOPE));
    public static final Issue ISSUE_ARG_COUNT =
            Issue.create("ClogArgCount", "Formatting argument types incomplete or inconsistent",
    ...
    public static final Issue ISSUE_ARG_TYPES =
            Issue.create("ClogArgTypes", "Formatting string doesn't match passed arguments",
    ...
    public static final Issue ISSUE_TAG_LENGTH =
            Issue.create("ClogTagLength", "Too Long Log Tags", "Log tags are only allowed to be at most"
